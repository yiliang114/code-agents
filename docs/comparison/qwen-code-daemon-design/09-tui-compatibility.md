# 09 — TUI 单进程 vs Daemon 兼容性

> [← 上一篇：协议兼容性](./08-protocol-compatibility.md) · [回到 README](./README.md)
...
详见 [§02 §7 双部署模式](./02-architectural-decisions.md#7-daemon-部署模式clihttpserver-vs-headlesshttpserver)。
...
| 能力 | dual-output | Mode A（[§02 §7](./02-architectural-decisions.md#7-daemon-部署模式clihttpserver-vs-headlesshttpserver)）|
|---|---|---|
...
| 跨 client capability 反向 RPC | ❌ | ✅（[§10](./10-remote-cli-mode.md)）|
...
> 问题：daemon 不启 TUI（[§02 §7 Mode B](./02-architectural-decisions.md#7-daemon-部署模式clihttpserver-vs-headlesshttpserver)），把渲染交给远端 TUI / Web client，是否减少 qwen-code 对 CPU / 内存的消耗？
...
[← 回到 README](./README.md) · [下一篇：远端 CLI 模式 →](./10-remote-cli-mode.md)
