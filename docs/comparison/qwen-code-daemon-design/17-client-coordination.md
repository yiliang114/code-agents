# 17 — 多端协调策略：subscriber 协议 / liveness / takeover

> [← 上一篇：远端 CLI 模式](./16-remote-cli-mode.md) · [回到 README](./README.md)
...
> **多端协调要点**（[§03 §2](./03-architectural-decisions.md#2-状态进程模型) "1 daemon = 1 session"下）：
>
...
- ❌ 全局 hard limit "1 CLI per session"（[§16 §一](./16-remote-cli-mode.md) 列出 7 个被破坏场景）
...
interface Subscriber {
  ...
  capabilities: ClientCapabilities           // 见 §16 §3.5
...
每个 kind 的 token scope 限制（[§21 §四 AuthZ](./21-orchestrator-multi-tenancy.md#四authentication--authorization)）：
...
每次 takeover 写入 [§21 §六 audit log](./21-orchestrator-multi-tenancy.md#六audit-log)：
...
### 12.3 跨 daemon pod failover ([§15](./15-high-availability.md))
...
| §15 HA failover | activeTyper 状态需 in-memory，failover 后重协商 |
| §16 远端 CLI 模式 + capability | 本章 subscriber 元信息是 capability 路由依据 |
...
[← 返回 README](./README.md) · [下一篇：长跑稳定性 →](./18-stability-and-longevity.md)
