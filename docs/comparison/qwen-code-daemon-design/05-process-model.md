...
> [← 上一篇：HTTP API 设计](./04-http-api.md) · [下一篇：权限流与认证 →](./06-permission-auth.md)
...
| 配 MCP server | MCP × N | per-daemon（详见 §03 §3） |
...
任意 client 发 prompt → 另一 client 通过 SSE 看到完整事件流（[§03 §1](./03-architectural-decisions.md#1-session-是否跨-client-共享)）
```
...
跨 daemon 不共享状态（详见 [§17 Orchestrator](./17-orchestrator-multi-tenancy.md)）。
```
...
下一篇：[06-权限流与认证 →](./06-permission-auth.md)
