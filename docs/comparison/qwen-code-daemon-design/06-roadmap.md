...
| **3** Read-only control plane + diagnostics | read-only status routes + preflight/env diagnostics + MCP guardrails (measurement, not full pool) | PR 12-14 — ✅ PR 12 [#4241](https://github.com/QwenLM/qwen-code/pull/4241) MERGED 2026-05-17 13:37；🔧 PR 13 [#4251](https://github.com/QwenLM/qwen-code/pull/4251) + PR 14 [#4247](https://github.com/QwenLM/qwen-code/pull/4247) OPEN (doudouOUC, 2026-05-17 15:49/14:58) | 1.5c read-only + chiga0 diagnostics |
| **4** Auth-gated mutation/control routes | **mutation gating helper** + memory/agents CRUD + approval/tools/init/MCP-restart + FileSystemService 边界 + safe file read + file write/edit + auth device-flow | PR 15-21 — ✅ PR 15 [#4236](https://github.com/QwenLM/qwen-code/pull/4236) MERGED 2026-05-17 12:10；🔧 PR 16 [#4249](https://github.com/QwenLM/qwen-code/pull/4249) + PR 18 [#4250](https://github.com/QwenLM/qwen-code/pull/4250) + PR 21 [#4255](https://github.com/QwenLM/qwen-code/pull/4255) OPEN (doudouOUC, 2026-05-17 15:06/15:19/17:26)；PR 17/19/20 ⏳ | 1.5c CRUD + 文件 routes |
...
| **PR 13** preflight + env diagnostics routes | `GET /workspace/preflight` + `GET /workspace/env`（**不是** 一个 generic diagnostics blob）；包含 daemon-locality notes + actionable `errorKind`：`missing_binary` / `blocked_egress` / `auth_env_error` / `init_timeout` / `protocol_error` / `missing_file` / `parse_error`。**Constraint**：所有 diagnostic payload across PR 12 + 13 + 14 共享同一 cell shape `{ kind, status, error?, errorKind?, hint? }`，SDK reducer 一套渲染所有 3 routes（否则 3 schemas 会 drift）（依赖 PR 12；详 [§04 §五 Runtime locality](./04-deployment-and-client.md#五runtime-locality--environment-contract)）| 🔧 **OPEN [PR#4251](https://github.com/QwenLM/qwen-code/pull/4251)** (doudouOUC, 2026-05-17 15:49 回头补 PR 14 之前跳过的 PR 13；+1893/-29 19 文件 **720/720 tests passing**；`GET /workspace/env` 永远从 `process.*` 答从不 consult ACP；`GET /workspace/preflight` daemon-level cells（Node version/CLI entry/workspace dir/ripgrep/git/npm）总渲染 + ACP-level cells（auth/MCP discovery/skills/providers/tool registry/egress）idle 时 emit `not_started` placeholder；**第一个 land closed `errorKind` 7-value taxonomy**（PR 14 复用）；新 `BridgeTimeoutError` typed class + `mapDomainErrorToErrorKind` helper；**5 strict invariants**：① `env_var` cell **永不带 value** (hard-assert `'value' in cell).toBe(false)`) ② proxy URL 走 `redactProxyCredentials` + `URL.host` 只露 `host:port` ③ whitelist + presence-only enumeration 非白名单 secret 不可见（regression-tested）④ `errorKind` closed 7-value union 未识别 → undefined 不 fabricate ⑤ 两 route 永不 spawn ACP child（bridge test `handles.length === 0`，延续 PR#4241 idle-aware 设计）；6 commit 拆分每 commit 独立 mergeable；out of scope: egress probe deferred to PR 14 (#4247) 避 every preflight outbound-connect；⚠️ 未含 `integration-tests/cli/qwen-serve-routes.test.ts`，**第六次 drift 风险**) |
| **PR 14** MCP resource guardrails | measurement-backed：MCP child/session budget + warnings 或 controlled refusal（**不是** full shared pool；依赖 PR 1, 12）| 🔧 **OPEN [PR#4247](https://github.com/QwenLM/qwen-code/pull/4247)** (doudouOUC, 2026-05-17 14:58 跳过 PR 13 直接挑 PR 14；+1188/-13 12 文件 **637/637 tests across 21 files**；**counter 在 core 不在 daemon** 防 race against multi-emission `mcp-client-update` event；**3-mode 矩阵** `enforce`/`warn`/`off`：`warn` over-reserve 让 operator 看 `liveCount > budget`，`off` 纯 observability；**slot reservation by name** 在 `Set<string>` 抗 reconnect / `runWithDiscoveryTimeout` 删 / health-monitor restart；**atomic check + reserve** before `await client.connect()` 防 `Promise.all` microtask 边界 interleave；新 CLI flag `--mcp-client-budget=N` + `--mcp-budget-mode={enforce,warn,off}` env passthrough to ACP child；新 capability tag `mcp_guardrails` modes `['warn', 'enforce']`；status `/workspace/mcp` 加 `clientCount`/`clientBudget`/`budgetMode`/`budgets[]` + per-server `disabledReason: 'budget'`；`subprocessCount = stdio + websocket` 与 PR#4205 baseline `pgrep -P` **交叉验证 cap**；refusal **config-declaration-order deterministic**（forward-compat note: 未来 scope hierarchy 切 lowest-precedence-first）；引 claude-code `filterMcpServersByPolicy` + `--mcp-*` flag 命名 + opencode `heap.ts` hysteresis primitive + claude-code telemetry per-transport breakdown 4 prior art；v2 PR 14b deferred typed SSE push events `mcp_budget_warning` (hysteresis 75%/37.5%) + `mcp_child_refused_batch`；⚠️ 未含 `integration-tests/cli/qwen-serve-routes.test.ts`，**第四次 drift 风险**（PR#4245 刚立 invariant 又被）) |
