...
| **PR 7** **minimal** daemon-stamped client identity | daemon assigns/stamps `clientId`；emitted events 使用 trusted `originatorClientId`；无 revocation；**header-based echo (`X-Qwen-Client-Id`) only**（防 client 自报伪造）；state-changing routes 拒绝 unknown clientId；live-session scoped（daemon 重启 / session 死 = id 丢失）；新 capability tag `client_identity`；adapter 不切换（只 ship primitive）（依赖 PR 3, 4）| 🔧 **OPEN [PR#4231](https://github.com/QwenLM/qwen-code/pull/4231)** (chiga0, 2026-05-17 06:56, +672/-63 10 文件, **测试 / 生产 ≈ 1.34x** (398/297 LOC), local 222 CLI + 76 SDK daemon tests passing；⚠️ 三套来源 lockstep 风险：变更未含 `integration-tests/cli/qwen-serve-routes.test.ts`，`caps.features` toEqual 数组需同步到 13 features，预测 wenshao 第一轮抓这个) |
