...
| **3** Read-only control plane + diagnostics | read-only status routes + preflight/env diagnostics + MCP guardrails (measurement, not full pool) | PR 12-14 — 🔧 PR 12 [#4241](https://github.com/QwenLM/qwen-code/pull/4241) OPEN (doudouOUC, 2026-05-17 10:35)；PR 13/14 ⏳ | 1.5c read-only + chiga0 diagnostics |
...
| PR | 内容 | 状态 |
|---|---|:---:|
| **PR 12** read-only status routes | `GET /workspace/mcp` + `/skills` + `/providers` + `/session/:id/context` + `/session/:id/supported-commands`；payload 必须 protocol-versioned + SDK 内 typed；**关键不变式**：workspace routes **不 spawn ACP** 即使 polled（idle workspace 返 `initialized: false`），避免 monitoring 保活 daemon；session routes 要求 live session（依赖 PR 2, 4）| 🔧 **OPEN [PR#4241](https://github.com/QwenLM/qwen-code/pull/4241)** (doudouOUC, 2026-05-17 10:35, +2112/-61 19 文件 423 tests passing；新 `status.ts` +173 LOC + 5 新 capability tags + SDK 5 个 typed payload types；`acpAgent.ts` +342 大改 + tests +274；⚠️ 文件列表未含 `integration-tests/cli/qwen-serve-routes.test.ts`，**三套来源 lockstep 风险**（同 chiga0 PR#4231 当时漏的同款，预测 wenshao 首轮抓——caps.features 需 17 → 22 features）) |
