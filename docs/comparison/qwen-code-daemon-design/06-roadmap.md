...
| Stage | PR | 内容 | 状态 |
|---|---|---|---|
| **Stage 1 TUI** | [PR#4260](https://github.com/QwenLM/qwen-code/pull/4260) | 新 CLI subcommand `qwen daemon-tui --daemon-url` + harness 测 prompt/cancel/model/permission（+248）| ❌ **CLOSED 2026-05-18 06:59**（chiga0 主动；wenshao 抓 240-line command-layer 测试缺口后，Stage 2 PR#4266 真接入 Ink TUI 已覆盖更彻底）|
| **Stage 1 channel** | [PR#4261](https://github.com/QwenLM/qwen-code/pull/4261) | `qwen channel start --daemon-url` flag + env vars；channel 改 depend on 结构 `ChannelBridge` interface，可注入 `AcpBridge`（默认）或 `DaemonChannelBridge`（opt-in）（+159/-43）| 🔧 OPEN draft + CHANGES_REQUESTED（channel 无 Stage 2 PR，**仅剩此 Stage 1 在飞**）|
| **Stage 1 IDE** | [PR#4263](https://github.com/QwenLM/qwen-code/pull/4263) | VS Code 新命令 `Qwen Code: Daemon Smoke Test`：stream daemon session 文本到 output channel + permission quickpick（+165）| ❌ **CLOSED 2026-05-18 06:59**（chiga0 主动；wenshao 抓 `outputChannel` JSDoc dep 不一致后，Stage 2 PR#4267 真注入 webview 覆盖）|
...
| **PR 16** memory + agents CRUD | `GET/POST /workspace/memory` + `/agents` + agent-type scoped `GET/POST/DELETE /workspace/agents/:agentType`；mutation paths gated + audited（依赖 PR 15, 12）| ✅ **MERGED 2026-05-18 06:27** [PR#4249](https://github.com/QwenLM/qwen-code/pull/4249) (doudouOUC, **15h20m 创新 daemon 项目最长 lifetime** 破 PR#4250 13h55m；**11 rounds review + 6 个 LLM model**：Copilot / gpt-5.5 / DeepSeek-v4-pro / glm-5.1 / qwen-latest / github-advanced-security + wenshao R1-R7；最终 +5318/-8 23 文件（review 翻 +2364 LOC，初版 +2954/-3）；wenshao R11 quote: "**fold-in cascade 2a → 2j has addressed every critical and high-impact finding**"——10 sub-fix iteration 是 daemon 项目最系统化修复 trail；7 routes + 2 cap tag (`workspace_memory`/`workspace_agents`) + 2 typed event (`memory_changed`/`agent_changed`) workspace-level fan-out via `bridge.publishWorkspaceEvent`；3 scope decision: 静态 `QWEN.md` only (auto-memory deferred PR 16.5) / agents POST split create+update 409 `agent_already_exists` / persistent audit log deferred PR 24；工程亮点：① CRUD-scoped `Config` Proxy stub fail-loud ② whitespace-only-append suppression 防 no-op POST 污染 ③ workspace-level fan-out + `knownClientIds()` snapshot) — 此前 PR open 时 OPEN 状态保留如下供参照 (doudouOUC, 2026-05-17 15:06, **+2954/-3 17 文件 项目第二大 PR**；7 routes + 2 capability tag `workspace_memory` + `workspace_agents` + 2 typed event `memory_changed`/`agent_changed` workspace-level fan-out via `bridge.publishWorkspaceEvent` + `knownClientIds()`；3 scope decision: **静态 `QWEN.md` only**（auto-memory 4-type taxonomy + MEMORY.md + extract/dream deferred PR 16.5）/ agents POST split create-only on `/workspace/agents` + update on `:agentType`（409 `agent_already_exists`，match `SubagentManager.createSubagent` vs `updateSubagent`）/ audit stamp `originatorClientId` on emitted events only（persistent audit ring deferred PR 24）；**工程亮点**：① CRUD-scoped `Config` Proxy stub 只暴露 `getSdkMode`/`getProjectRoot`/`getActiveExtensions`，其他方法 throw 立即 crash —— fail-loud 防 future refactor 在 CRUD path 加方法 daemon silently incorrect ② **whitespace-only-append suppression** in `writeContextFile.ts` 防 no-op POST bump mtime / fan out 误导 `memory_changed`（返 `{ changed: boolean }` flag）③ `bridge.publishWorkspaceEvent` per-entry catch + `writeServeDebugLine`；**PR open 前自跑 2 specialist agents** (code-review + silent-failure-hunter) **5 项 high-impact pre-fix**——review pipeline 推前到 PR open 前；693 tests green (348/348 serve + 10/10 writeContextFile + 335/335 SDK)，30+ new `it` blocks 覆盖 success/strict-deny/4xx/event-stamp/fan-out/SDK round-trip；⚠️ **第五次 drift 风险**：未含 `integration-tests/cli/qwen-serve-routes.test.ts`，doudouOUC 故意 batch fix 等多 cap 合后一次 follow-up 校准) |
| **PR 17** approval + tools + init + MCP restart controls | `POST /session/:id/approval-mode` + `/workspace/tools/:name/enable` + `/workspace/init` + `/workspace/mcp/:server/restart`（依赖 PR 15, 12, 14）| ✅ **MERGED 2026-05-18 16:27** [PR#4282](https://github.com/QwenLM/qwen-code/pull/4282) (doudouOUC, **6h21m + 34 reviews**；最终 +3685/-13 28 文件 (review +939)；**4 strict-gated mutation routes**：`POST /session/:id/approval-mode` (`plan`/`default`/`auto-edit`/`yolo`) + optional `persist: true` / `POST /workspace/tools/:name/enable` (`tools.disabled` skip-register vs `permissions.deny` runtime gate 区分) / `POST /workspace/init` **故意纯机械** scaffold `QWEN.md` 不调 model (保 mutation 同步语义 vs prompt 异步语义清晰；AI-fill 需显式 follow-up `POST /session/:id/prompt`) / `POST /workspace/mcp/:server/restart` + PR 14 v1 budget pre-check；全 strict-gated by PR 15 + `X-Qwen-Client-Id` audit + `originatorClientId`-stamped SSE event；**核心 hardening**：`TrustGateError` typed class in core → bridge map untrusted-folder rejection to `errorKind: 'auth_env_error'` (PR 13 taxonomy) **不再 regex-match messages**——跨 PR typed-error 模式复用 (同 PR 13/PR 14 `errorKind` 自动分类)；wenshao 14:59 CHANGES_REQUESTED → 15:51 doudouOUC inline 修 → 16:27 APPROVED + MERGED) |
...
| **PR 19** safe workspace file **read** routes | read/list/stat only；经 `FileSystemService` 路由；canonicalize paths + workspace boundary + size/binary limits + symlink policy + actionable errors（依赖 PR 18）| ✅ **MERGED 2026-05-18 08:17** [PR#4269](https://github.com/QwenLM/qwen-code/pull/4269) (doudouOUC, **1h54m 极短 lifetime**——PR 18 chokepoint 红利显化；最终 +1454/-8 10 文件；wenshao 中文 APPROVED quote: "本地实测通过(525/525 tests + **安全边界全部正确拒绝** + **审计串联确认**)"；PR-internal lockstep 已 internalize) — 此前 PR open 时 OPEN 状态保留如下供参照 (doudouOUC, 2026-05-18 06:23 **PR#4249 merge 前 4 分钟开零空窗接力**；+1177/-3 10 文件；4 routes 通过 PR 18 `WorkspaceFileSystem` boundary：`GET /file?path=...` UTF-8 text + meta(encoding/BOM/lineEnding) + `?maxBytes` soft-truncate + `?line`+`?limit` window / `GET /list?path=...` `{name, kind, ignored}` 2000-entry cap + `?includeIgnored=1` opt-in / `GET /glob?pattern=...` workspace-relative + `?cwd` + `?maxResults` / `GET /stat?path=...`；单 capability tag `workspace_file_read` 覆盖 4 read route；wire `runQwenServe` 构 + inject `fsFactory`——**PR 18 立的 `trusted: false` default invariant 在 PR 19 验证**(第一个真 consumer)；关闭 PR 18 follow-up #2 + #4：`fsFactory` injection contract test + glob audit hashes `boundWorkspace` 但 emit 文字 `pattern` field on 成功 + parse-rejection 两 path；trust snapshot 默认 `true`(operator-chosen workspace)) |
| **PR 20** file write/edit routes behind auth | 独立 PR：mutation gate + audit log + trust/qwenignore + 显式 symlink policy（依赖 PR 8, 15, 19）| ✅ **MERGED 2026-05-18 14:37** [PR#4280](https://github.com/QwenLM/qwen-code/pull/4280) (doudouOUC `[codex]`, **4h47m + 仅 13 reviews**——PR#4255 巅峰后回归常态；最终 +2557/-266 25 文件；4 块新功能：① bounded raw byte reads (补 PR#4269 漏的 binary) ② hash-bearing text reads (给 write 用做 precondition) ③ strict-auth text write+edit routes 经 PR 15 mutation gate ④ content-hash concurrency checks 防 stale write；**4 关键工程亮点**：① **serve-local atomic temp+rename helper**——写不直接 truncate，先 temp file → fsync → hash precondition → rename → fsync dir，防中断破坏文件 ② **expected-hash precondition** 防 stale write 丢失更新 ③ **edit single-match policy**——`oldText` 必须 **exactly 1 处** match（零/多 match 都报错让 caller disambiguate）④ `X-Qwen-Client-Id` 强制 stamp audit on write/edit；12:58 wenshao CHANGES_REQUESTED → 13:33 doudouOUC 3 inline 修 → 14:17 gpt-5.5 /review APPROVED → 14:37 MERGED；follow-up PR#4279 Windows hotfix 已先修 PR#4269 的 `path.sep` 漏 `/` normalize) |
| **PR 21** auth device-flow route | `POST /workspace/auth/device-flow` 或 Capability RPC for remote auth；必须 honor runtime locality（依赖 PR 15, 12）| ✅ **MERGED 2026-05-18 14:05** [PR#4255](https://github.com/QwenLM/qwen-code/pull/4255) (doudouOUC, **20h39m daemon 项目史上最长 lifetime** 破 PR#4249 15h20m + **135 reviews 史上最多** + 最终 **+6172/-51 daemon 项目史上最大 PR** 破 PR#4249 +5318；review 期间几乎翻倍 +2645/-43；OAuth 2.0 Device Authorization Grant (RFC 8628) brokered through daemon；**4 route 在 `/workspace/auth/`**：POST device-flow strict + idempotent take-over / GET (5-min terminal grace) / DELETE idempotent / GET status；**5 workspace-scoped typed events** + new cap tag `auth_device_flow`；**Runtime locality 严格**：daemon NEVER spawn browser——**build-time static-source grep test** 11 forbidden patterns；**8 设计精彩点**：① per-providerId singleton + concurrent-call coalesce ② **`BrandedSecret<T>` 4-way redaction** 防 `Symbol.toPrimitive` leak + 6 leak-path tests ③ `persist()` disk write FIRST 防 zombie token ④ `poll()` accepts signal + lost-success branch audit ⑤ `transitionTerminal` boolean 防 double-fire ⑥ `broadcastWorkspaceEvent` distinct PR 16 ⑦ `BridgeClient.dispose()` wired into shutdown drain ⑧ `oauth_creds.json` 0o600；**临门一脚 1 TS inference 卡 12:51** (`daemonEvents.test.ts:664` synthetic `slow_client_warning` `type: string` 推导不匹配 `DaemonEvent`) → 13:01 修 → 13:59 gpt-5.5 /review LGTM → 14:05 MERGED；3 specialist agent pre-run 12 P0/P1 fold-in 加 `type-design-analyzer`) |
...
| **PR 23** `feat(mcp): shared MCP transport/process pool` | 真共享 pool，keyed by canonical workspace + server **config hash** + auth/env/runtime inputs；lifecycle/refcount tests（依赖 PR 22, 14）| ✅ **F2 MERGED** [#4336](https://github.com/QwenLM/qwen-code/pull/4336) 2026-05-21 15:56 to `daemon_mode_b_main`（doudouOUC, ~40h, merge `46f8d48f`, +10308/-147 38 files, 22 commits / 247 reviews；6 feature commit 全落地 + 16 review fold-in；解 [PR#4306 揭示的 double MCP discovery 架构发现](#)；详 #F2-PR-23 段 |
| **PR 24** `feat(security): client pairing revocation + PermissionMediator` | pair tokens + revocation API + audit log + 4 policy strategies（first-responder / designated / consensus / local-only）（依赖 PR 8, 22）| 🟢 **拆 2 阶段** — **F3** [#4335](https://github.com/QwenLM/qwen-code/pull/4335) ✅ MERGED 2026-05-20 11:13 to `daemon_mode_b_main` (`8eeb5100`, +9748/-517 62 files) —— 4 strategies 实现 + audit ring + 2 新 SSE 事件 + 3 typed errors + SDK reducer 扩展；pair-token + revocation API 推迟为后续 follow-up（consensus 需要的 voter auth）|
...
✅ **MERGED 2026-05-19 01:27** (doudouOUC, **1h57m** open→merge, +852/-371 11 files, merge commit `68e3ec988a`；wenshao 1 轮 CHANGES_REQUESTED → 4 轮 inline 修 → qwen-latest /review LGTM → MERGED)。**3-stage split 改 4-stage** —— 原 PR 22b/2 (implementation lift, ~3000 LOC) 拆为 22b/2 (design slice 凝结契约) + 22b/3 (机械 bulk lift)，让 review attention 集中在 6 个 design decision 上。
...
#### PR 22b/3（合并打包为 F1）— mechanical bulk lift + 22b' BridgeFileSystem 注入接口

**maintainer 重组（2026-05-19）**：剩余 Mode B 工作不再走 main，而是拆为 **F1-F5 feature PRs target 长期 integration 分支 `daemon_mode_b_main`**。F1 把原 PR 22b/3 + 22b' 合并打包，让 `@qwen-code/acp-bridge` 变成完整、可注入、可测试的独立包。

**F1 ([#4319](https://github.com/QwenLM/qwen-code/pull/4319)) 内容**：

| 步 | commit | 范围 | LOC |
|---|---|---|---|
| 1 | `ce2c338d` | `defaultSpawnChannelFactory` + `killChild` + `SCRUBBED_CHILD_ENV_KEYS` → `acp-bridge/spawnChannel` | ~283 |
| 2 | `37fb238f` | `BridgeClient` class + `PendingPermission` + `PermissionResolutionRecord` + `MAX_RESOLVED_PERMISSION_RECORDS` + 早期事件常量 + `describeStatKind` + `sliceLineRange` → `acp-bridge/bridgeClient` | ~700 |
| 3 | `a84d817a` | `createHttpAcpBridge` factory closure + factory-only helpers (`withTimeout` / `canonicalizeExistingAncestor` / `verifyParentWithinWorkspace` / debug helpers / `hasControlCharacter`) + `ChannelInfo` + `SessionEntry` interfaces + factory constants → `acp-bridge/bridge` | ~3000 |
| 5 | `b6516fc0` | **新 `BridgeFileSystem` 接口** + `BridgeOptions.fileSystem` 字段 + `BridgeClient` constructor 注入 + early-return 委托 in `writeTextFile`/`readTextFile`（原 22b'）| ~77 + seams |

**净效果**：`cli/src/serve/httpAcpBridge.ts` **4682 LOC → 97 LOC** thin re-export shim（channels/base/AcpBridge.ts + VSCode IDE companion 可直接消费 acp-bridge，无需跨 package reach 或 reimplement child lifecycle）。

**设计抉择**：
- **`BridgeClientSessionEntry` minimal interface**（step 2）—— factory 的 richer `SessionEntry` 是 daemon-host bookkeeping，BridgeClient 只读 4 字段；minimal interface 保 acp-bridge 不污染 session-management types
- **`writeStderrLine` inlined per-file**（steps 1-3）—— 每个 lifted file 内联 3-line helper 而非 reach back to `cli/src/utils/stdioHelpers`，保 zero reverse-dep
- **`BridgeFileSystem` optional**（step 5）—— 缺省时 BridgeClient fall back inline `fs.writeFile/readFile` proxy，pre-F1 行为 verbatim 保持（tests + Mode A consumers + channels + IDE companion 都不变）

**验证**：tmux 9-step f1-verify.sh → **9/9 PASS** at HEAD `c4bcd6da1`，**1221 tests across 35 files**（62 acp-bridge / 735 cli serve / 424 sdk-ts）+ 全 11 packages clean。
...
| 编号 | 范围 | LOC |
|---|---|---|
| **#4329 → 折叠** | `normalizeDisabledToolList` helper extraction folded in commit `3a6bf3bfd` | - |
...
✅ **MERGED 2026-05-21 15:56** to `daemon_mode_b_main`（doudouOUC, ~40h open→merge, merge commit `46f8d48f`, 最终 +10308/-147 38 files, **22 commits = 6 atomic feature + 16 review fold-in**, **247 reviews / 30 CHANGES_REQUESTED rounds**）。existing standalone qwen + per-session McpClient path **untouched**（71/71 mcp-client-manager tests pass unchanged）→ 非 daemon 路径零影响。

**解 [PR#4306 揭示的 "double MCP discovery in ACP child"](#) 架构发现**——`runAcpAgent` bootstrap + per-session `newSessionConfig` 各跑独立 `McpClientManager` 的 2× spawn cost 浪费，F2 用 **workspace-scoped pool sharing** 修：N session 共享 `(name + fingerprint)` 一个 transport，4 session × `--mcp-client-budget=2` workspace 封顶 2 而非 8。

**6 feature commit**：
| # | 内容 |
|---|---|
| 1 | `refactor(core): split McpClient.discover into pure tool/prompt list` |
| 2 | `feat(core): McpTransportPool + SessionMcpView` —— pool core / fingerprint key / refcount / drain state machine / snapshot replay / generation guard |
| 3 | `feat(core): cross-platform pid sweep` —— Linux/macOS `pgrep -P` + Windows PowerShell CIM |
| 4 | `feat(serve): wire McpTransportPool into QwenAgent daemon mode` —— Config/ToolRegistry/McpClientManager threading + `acpAgent.QwenAgent.mcpPool` ctor + pool lifecycle hook SIGTERM/IDE-close |
| 5 | `feat(serve): pool-aware status + restart routes` —— `GET /workspace/mcp` 加 `entryCount`+`entrySummary` / restart route 加 `?entryIndex=` / **新 capability tag `mcp_workspace_pool` + `mcp_pool_restart`** |
| 6 | `feat(serve): graduate MCP budget guardrails to workspace scope` —— 新 `WorkspaceMcpBudget` class + `scope:'workspace'` cell + SDK `isWorkspaceScopedBudgetEvent` helper |

**关键不变式**（design doc `docs/design/f2-mcp-transport-pool.md` v2.2）：

| 不变式 | 内容 |
|---|---|
| Workspace-scoped pool | N ACP session 共享 `(name + fingerprint)` 一个 transport；`--mcp-client-budget` 是 workspace 级封顶 |
| Fingerprint key §5.1 | SHA-256 over canonical config；per-session filters (`includeTools`/`excludeTools`/`trust`) **excluded**；OAuth normalized via `canonicalOAuth` |
| HTTP/SSE opt-in §5.2 | default `pooledTransports = {stdio, websocket}`；HTTP/SSE 走 `createUnpooledConnection` unless operator 通过 `QWEN_SERVE_MCP_POOL_TRANSPORTS` 显式 opt in |
| `spawnInFlight` dedupe §6.2 | 5 concurrent acquires for same key → 1 spawn（测试钉） |
| Drain grace + max-idle hard cap §6.3 | default 30s drain，5min hard cap 抗 flap；attach **和** `restart()` 时都 cancel（W4/W31 fix） |
| `sessionToEntries` reverse index | `releaseSession` O(refs) not O(entries) |
| Spawn-failure slot release | catch-arm 用 `parseConnectionId` 释放 reserved slot，不用 `startsWith`（W21 false-positive fix） |
| Snapshot replay on attach §7.2 | 同步 `applyTools`/`applyPrompts` 防 late attacher 见 empty state；`skipReplay` gate for unpooled bypass path |
| Generation guard §7.3 | stale `tools/list_changed` handlers from pre-reconnect Client instances 不覆盖 post-reconnect snapshots |
| Per-session trust copy §7.1 | `tool.withTrust(this.cfg.trust)` clone；trust 不变时 allocation pin |
| Cross-platform descendant pid sweep §6.4 | Linux/macOS `pgrep -P` recursive / Windows PowerShell CIM（`$p` 变量绑定 W5 defense-in-depth）；bounded 2s timeout / 256 max / 8 depth；graceful degradation |
| Workspace budget §11 | hysteresis 75% upward fire / 37.5% re-arm；refused-batch coalescing per discovery pass；`scope:'workspace'` 事件经 `broadcastBudgetEvent` fan out 到所有 attached session |

**"review at implement" 工作流**：早期 commit 2 写完后 background 跑 `code-reviewer` agent，**继续 implement commit 3 期间并行 review**，抓 2 P1 bugs（`PooledConnection.release()` no-op 漏 refs / `createUnpooledConnection` 双 teardown）fold 到 commit 3。最终 22 commits 里 16 个是 review fold-in（`fix(core): wenshao Wxx review fold-ins ...`）—— 比 daemon 项目之前的 "review at merge" → "review continuously" 更进一步。

#### PR 26 → F4 + F4 prereq ([#4360](https://github.com/QwenLM/qwen-code/pull/4360)) — client adapter wave + 协议层前置
...
| stamping | 位置 | 说明 |
|---|---|---|
| `_meta.serverTimestamp` | `server.ts formatSseFrame()` SSE write boundary | spread-merge 保留已有 `_meta` key（如 tool_call 的 `toolName`）；SDK 走 `_meta.serverTimestamp` 读（Anthropic convention）|
| `errorKind` on `stream_error` | `server.ts` `stream_error` emit | via `mapDomainErrorToErrorKind(err)`；unclassified → undefined → 字段省略（strictly additive）|
| tool `provenance` | `ToolCallEmitter.ts` 新静态 `resolveToolProvenance(toolName, subagentMeta)` | → `{provenance: 'builtin'\|'mcp'\|'subagent'; serverId?}`；emitStart + emitResult + emitError **都 stamp**（reconnecting client 可从任何 update event 重新 derive provenance）；用 `mcp__<server>__<tool>` naming heuristic |

**NOT stamped**：`session_died` 的 `errorKind` —— `acp-bridge/bridge.ts` 3 个 `session_died` emit site 无 classifiable `err` in scope（`channel_closed` 只带 exitCode/signalCode；`killed` + `daemon_shutdown` 是 user/operator-initiated 非 domain error）；threading channel-spawn error 进来以启用 `init_timeout` / `missing_binary` 分类留单独 PR，避免混 protocol stamping 与 lifecycle plumbing。

**commit 2 `c1a2f0a78` — #15 SSE reducer gap detection**（关 Ilya0527 [#4175 comment #15](https://github.com/QwenLM/qwen-code/issues/4175) raise 的 multi-client state divergence bug）：

- **daemon `eventBus.ts`**：`subscribe()` replay path 检测 ring eviction（`ring[0]?.id` vs `lastEventId + 1`）。`earliest > last + 1` 时 force-push synthetic terminal frame `state_resync_required`（`{v:1, type:'state_resync_required', data:{reason:'ring_evicted', lastDeliveredId:N, earliestAvailableId:M}}`）—— **无 `id`，mirror `client_evicted` synthetic terminal pattern**；replay 在它之后继续（network-friendly，不强制 reconnect）
- **SDK `events.ts`**：加 event type 到 `DAEMON_EVENT_TYPES` + data interface + predicate + widen `DaemonStreamLifecycleEvent` + reducer state fields (`awaitingResync` / `resyncRequiredCount` / `lastResyncRequired?`)；`awaitingResync === true` 时在 reducer 顶部 gate non-terminal events；`RESYNC_PASSTHROUGH_TYPES` set 记录哪些 event type 仍 apply（terminal lifecycle frame 必须 —— 让 resync limbo 期间死的 session 仍 observable）
- **consumer recovery contract**：`state.awaitingResync === true` → consumer 调 `loadSession`（out of band，SDK 看不到这是个 event）→ `createDaemonSessionViewState({...seedFromLoadSession})` 重建 view state，fresh state 默认 `awaitingResync: false` 隐式清 flag

**全 additive backward-compat**：旧 SDK 忽略 `_meta.serverTimestamp` / `provenance` / `serverId` / `errorKind`；旧 daemon 不 emit `state_resync_required`，SDK reducer switch fall-through 忽略 unknown type（per `unrecognizedKnownEventCount` 语义）。4 个 design decision 都 confirm via `AskUserQuestion` before coding。tests +27（server +3 / ToolCallEmitter 46 含 +11 / acp-bridge 108 含 +5 / sdk 451 含 +8）。

### Wave 6 — Release hardening + v0.16

> ⚠️ **产品版本 v0.16.0 已发布（2026-05-21，[PR#4404](https://github.com/QwenLM/qwen-code/pull/4404)）≠ daemon Wave 6 "PR 31 v0.16 production-ready release" 里程碑**。
> v0.16.0 是 qwen-code **产品** semver 版本号 bump（0.15.11 → 0.16.0，含 root + cli + core + **acp-bridge** + channels×5 + vscode-ide-companion + web-templates + webui 全 workspace 包）。它**在 main 上**包含 daemon **Wave 1-4 全部 + Wave 5 PR 22a/22b/1/22b/2**（这些都已合 main）；但 **Wave 5 F-series（F1/F2/F3 + F4 prereq）在 `daemon_mode_b_main` 集成分支上，尚未周期 merge 回 main → 不在 v0.16.0**。
> 因此 daemon Mode B 在 v0.16.0 中是 **Wave 1-4 形态**（HTTP routes / capability registry / mutation gate / OAuth device-flow 等），架构抽取（acp-bridge 完整自足 / MCP shared pool / PermissionMediator）随 F-series 后续随周期 merge 进入下一个产品版本。daemon Wave plan 自身的 "PR 31 v0.16 production-ready" 是 daemon Mode B **功能完备宣告**，与产品 semver v0.16.0 是两件事。
...
| **web/debug** | [PR#4132](https://github.com/QwenLM/qwen-code/pull/4132) `/demo` ✅ **MERGED 2026-05-18 08:31** (jifeng, 4 天 24 reviews) | 作为最薄 POST+SSE client 验证面，已 ship；XSS 修把威胁模型扩到 daemon-emitted poisoned events | 已 ship |
