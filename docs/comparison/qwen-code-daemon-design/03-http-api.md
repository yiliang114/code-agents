...
# Workspace 管理（daemon 内 byWorkspaceChannel 管 M workspace；跨 daemon process 才需 External orchestrator）
...
**Orchestrator API base URL** 由 `coordinator.baseUrl` 配置（独立于 daemon URL）。Mode A（CLI + HttpServer）单用户场景 **不需要 orchestrator**——直接连 daemon URL（`qwen --serve` 启动时打印的端口）。Orchestrator 仅在**跨 daemon process 部署**（多 daemon process 池 / 多 tenant 隔离 / 跨机器 SaaS）时启用——单机单 daemon Mode B 部署 client 直连即可（commit `6a170ef8` 后单 daemon 已支持 M workspace × N session）。
...
**Daemon 层 0 wire 破坏（PR#3889 已实现的全保留）+ Orchestrator 层全新一套（External Reference Architecture，由外部实施）**。**单机部署**（Mode A 或单 daemon Mode B，无论 1 session 还是 N session × M workspace）跳过 orchestrator 直连 daemon URL；**跨 daemon process 部署**（多 daemon process 池 / 多 tenant 隔离 / 跨机器 SaaS）需要 orchestrator 路由 sessionId → daemonUrl。SDK 加 `coordinatorUrl` 配置项区分两种部署。
