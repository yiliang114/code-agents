...
>
> 代价：多 daemon 实例（不同 workspace 或 orchestrator 起的多 session）时，LSP / MCP children 重复 spawn。N ≥ 50 时可投资源池化（用户级 LSP daemon / 共享 MCP，属于 External SaaS 资源池化范畴）；MCP children 通常很轻。

## 1. MCP server 共享策略

### 1.1 决策回顾（[03 §3](./03-architectural-decisions.md#3-mcp-server-生命周期)）

**per-daemon MCP state**——每个 daemon instance 持有自己的一套 MCP client 集。**不跨 daemon 共享**。OpenCode multi-session 模型下走 per-workspace（`packages/opencode/src/mcp/index.ts`）；qwen-code 1 daemon = 1 session 模型下边界天然落在 daemon 进程层。
...
### 2.2 MCP / FileReadCache 都是 per-daemon

1 daemon = 1 session 模型下，MCP / FileReadCache 都绑在 daemon 进程上 —— daemon 退出全部清理，无需 per-session 路由。共享语义统一：

| 资源 | 共享边界 | 状态泄漏后果 |
|---|---|---|
| MCP server | per-daemon | daemon 内一组 children，跨 daemon 不共享 |
| FileReadCache | per-daemon | session 私有等价于 daemon 私有（PR#3774 prior-read 守卫语义自然成立）|
...
| MCP server | per-daemon| 同 daemon 内并发 reconnect coalesce + 30s 健康检查（决策 §3）| PR#3818（PR#3819 已 closed）|
