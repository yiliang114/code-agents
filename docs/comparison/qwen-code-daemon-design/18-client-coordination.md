...
每个 kind 的 token scope 限制（[§22 §四 AuthZ](./22-orchestrator-multi-tenancy.md#四authentication--authorization)）：
...
每次 takeover 写入 [§22 §六 audit log](./22-orchestrator-multi-tenancy.md#六audit-log)：
