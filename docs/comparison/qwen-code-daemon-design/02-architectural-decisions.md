...
### 1 daemon = 1 workspace 后为什么仍不共享 cache

[PR#4113](https://github.com/QwenLM/qwen-code/pull/4113) 把 daemon 收紧为 1 daemon = 1 workspace × N session 后，cache key 不再需要 workspace 维度（之前 multi-workspace 时 `(workspace, dev, ino)`，现在 `(dev, ino)` 即可）——**简化了 key 但不改变共享语义**。4 项反对依据**与 workspace 维度无关，仍然成立**：

| # | 反对依据 | 与 workspace 维度的关系 |
|---|---|---|
| 1 | **PR#3774 prior-read enforcement 语义**：`EDIT_REQUIRES_PRIOR_READ` 意思是 "**当前 session** 未读过 → 拒绝 Edit"。共享后 sessionA read 过 → sessionB 可以 Edit（即使从未 read）—— 违反 per-session agent autonomy 约束 | ✅ 与 workspace 无关 |
| 2 | **PR#3810 invalidation 5 路径**：5 路径都是 per-session 触发。共享后 sessionA 的 microcompact 触发 invalidation → sessionB 受影响 | ✅ 与 workspace 无关 |
| 3 | **LLM token 节省效果不大**：FileReadCache 真正价值是节省"同 file 重复发给 LLM"的 token，但 LLM context 是 **per-session**——同 file 在 sessionA 给 LLM 一次，sessionB 仍要给自己 LLM 一次。共享只省 disk I/O 不省 token | ✅ 与 workspace 无关 |
| 4 | **OS page cache 已兜底**：disk I/O 应用层已 ms 级 | ✅ 与 workspace 无关 |

**Stage 2e 候选优化（不在 PR#4113 范围）**：理论上可拆 2 层 — "content cache（`path+mtime→content bytes`）跨 session 共享" + "per-session read set（`Set<fileId>` 保留 PR#3774 enforcement）"。但 OS page cache 已兜底 disk I/O，增益边际；实现复杂度（拆 unified FileReadCache + 拆 PR#3810 invalidation 路径）较高。**仅在 500+ session/机实测瓶颈时再考虑**。
