...
| 所有 client 都断开 + 空闲 N 分钟 | daemon instance 进入 idle，可被 orchestrator 回收（详见 §18）|
...
适用边界：单机 N < 50 并发 session 经济性可接受；N ≥ 100 时考虑资源池化或迁移到多 session 模式（详见 [§20 设计对比](./20-single-vs-multi-session-design.md)）。
...
| **mDNS 广播**（§16） | 可选 `--discoverable` flag | 可选配置 `discovery.mdns: true` |
