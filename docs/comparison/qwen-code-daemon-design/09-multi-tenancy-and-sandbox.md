...
> **本章只关注 daemon 内的 sandbox 设计**。多租户 ACL / 配额 / 审计 / OIDC 等 orchestrator 层事项见 [§15 Orchestrator 多租户与配额](./15-orchestrator-multi-tenancy.md)。
...
**多租户 ACL / 配额 / OIDC** 在 orchestrator 层（[§15](./15-orchestrator-multi-tenancy.md)），不在本章范围。
