...
| 🔒 **安全 / 多租户专题** | ~2 h | §11 → §21 → §12 → §15 → §17 | 企业部署评估 |
| 🌐 **远端 / 协作专题** | ~2 h | §13 → §16 → §17 | 客户端体验设计 |
| 💾 **数据架构专题** | ~1 h | §14 → §21 → §15 §三-§九 | 持久化 / HA 设计 |
...
| 16 | [远端 CLI 模式与 Client Capability 协议](./16-remote-cli-mode.md) | 3 类拓扑（Local-Local / Local-Remote 不推荐 / **Remote-Remote 推荐**）+ Client Capability 反向 RPC 协议 + 5 类 capability + TLS/mTLS auth + NAT 穿透 + Local echo + VSCode Remote-SSH 对比 |
| 17 | [多端协调策略](./17-client-coordination.md) | 不限同类型 client 数量（保 collaboration 哲学）+ 6 类 client kind 分桶上限 + liveness（30s heartbeat / 90s 超时 / TCP RST 即时剔除）+ active typer 协调 + 显式 takeover + 可选 exclusive_per_type 模式 + IM bot 一对多用户 |
...
| 21 | [Orchestrator 多租户 + 持久化栈](./21-orchestrator-multi-tenancy.md) | **multi-tenancy 在 orchestrator 层** —— Tenant 抽象 / AuthN 4 模式（Bearer / OIDC / mTLS / cookie）/ AuthZ workspace 映射 / Quota engine（Redis 原子 + reservation 模式）/ Audit log 4 通道（jsonl / syslog / OpenTelemetry / Kafka）+ **持久化栈完整设计**（SQLite 边界 / drizzle-orm 选型 / 8 张核心表 schema / Storage Adapter 抽象 / External Phase 4 多 daemon 共享 / 迁移 / 安全 / 性能基准）|
...
| 15 | [HA 高可用与故障恢复](./15-high-availability.md) | **5 层 HA 架构**（Edge DNS → Ingress sticky → StatefulSet pod N≥3 → Postgres Patroni + Redis Sentinel + S3 多 AZ）+ SSE Last-Event-ID 重连协议 + LLM streaming 中断 7 类场景 + 90s graceful drain + 15 项 Chaos 测试 + 99.9% SLO |
| 18 | [长跑稳定性与可观测性](./18-stability-and-longevity.md) | **接受"重启不可避免"** —— Node.js 长跑 7 类风险（heap / GC / fd / zombie / exception / native crash / ALS 链表）+ 多租户加剧 5 类 + qwen daemon 10 个具体泄漏点（含修复代码）+ **9 项稳定性模式**（TTL / bounded / quota / circuit breaker / memory threshold restart / heap dump / liveness / native supervisor / worker isolation）+ 6 类 native module 风险 + 22 项 Prometheus 指标 + 30 天 Soak/Chaos 测试矩阵 + Bun vs Node.js 长跑实测 |
...
| # | 文档 | 一句话 |
|---|---|---|
| 08 | [路线图](./08-roadmap.md) | qwen-code 主线 ~3 周内 feature complete：Stage 1（~1 周 ✅ Mode B headless PR#3889）/ Stage 1.5（~4d Mode A）/ Stage 2（~1-2 周 mDNS+OpenAPI+WebSocket+多 token+metrics）。**Orchestrator / 多租户 / 沙箱 / SaaS 部署作为 External Reference Architecture**（参考设计 §20 / §21 / §11 / §15），由外部商业平台 / k8s operator 实现 |
| 09 | [与 OpenCode 详细对比](./09-comparison-with-opencode.md) | 路由 / 技术栈 / 设计哲学逐项对照 |
| 19 | [与 Anthropic Managed Agents 对比](./19-vs-anthropic-managed-agents.md) | **5 层架构对照**（client / agent runtime / tool / sandbox / persistence）+ **内置工具映射** + **协议层差异**（Anthropic 私有 vs ACP 标准）+ **双向 migration path**（Anthropic→Qwen / Qwen→Anthropic 兼容 API）+ **6 类客户场景推荐** + **决策树 6 问选型** + **3 种混合部署模式** + **"Managed Qwen Agents" 产品蓝图**（基于 External Reference Architecture 完整实施 (External Phase 1-4) 包装，6 月可建）|
| 20 | [单 vs 多 Session 设计深度对比](./20-single-vs-multi-session-design.md) | **22 维对比矩阵 + 6 项关键 tradeoff 深度分析**（隔离昂贵性 / cold start 平方根 / 内存 baseline 建模 / 隔离失败代价 / 复杂度守恒原理 / PR#3889 现实约束）+ **决策树 N≤5/50/100/500/500+** + 作为选型决策入口（不在主线扩展多 session 模型）|
...
- 持久化：每 daemon 自己的 transcript JSONL；外部 orchestrator 可选 SQLite/Postgres 做 cross-daemon 聚合（详见 [§21 持久化栈](./21-orchestrator-multi-tenancy.md#八引入-sqlite-的边界external-phase-1-orchestrator-层)）
...
| 多 client 并发请求 | [§03 决策](./03-architectural-decisions.md) §6 FIFO + fan-out + first responder + [§17 多端协调](./17-client-coordination.md) |
| 实体层级 | [§14 实体模型](./14-entity-model.md) 5 层 hierarchy + 认证侧 |
| 持久化 | [§21 持久化栈](./21-orchestrator-multi-tenancy.md)（持久层） JSON → SQLite → Postgres 演进 |
| 多租户 / 沙箱 | [§11 Shell 沙箱](./11-multi-tenancy-and-sandbox.md) + [§21 Orchestrator 多租户](./21-orchestrator-multi-tenancy.md) + [§12 越权防御](./12-horizontal-privilege-defense.md) |
| HA / SaaS 部署 | [§15 高可用](./15-high-availability.md) |
| 远端 CLI / 协作 | [§16 远端 CLI](./16-remote-cli-mode.md) + [§17 多端协调](./17-client-coordination.md) |
...
| **PR#3739** ✓ | Background agent resume + transcript-first fork resume | daemon 重启 / failover 后 session 可恢复（[§15 §五](./15-high-availability.md) SSE 重连协议的隐藏基础设施）|
...
| 第四轮 | §14-§15 | 实体模型 + 持久层 + HA |
| 第五轮 | §16-§17 | 远端 CLI + 多端协调（client capability 协议）|
