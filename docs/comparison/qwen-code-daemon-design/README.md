...
- 🎉 **Wave 1+2+2.5+3+4 + W5 PR 22a+22b/1 + multi follow-up 同夜冲合**（2026-05-16~18 共 **35 MERGED + 5 OPEN/draft + 3 CLOSED**：22 Wave PRs + 7 follow-up + 3 bonus Stage 0 + 1 外部 + 2 refactor follow-up；PR 17 P2 fold-in + telemetry polish + 3 adapter draft；Wave plan 进度 **22.5/31 ≈ 73%**）：
  ...
  - 🔧 **F1 follow-up** [PR#4334](https://github.com/QwenLM/qwen-code/pull/4334) BridgeFileSystem wiring + channelInfo fix **OPEN REVIEW_REQUIRED** 2026-05-19 17:20 (doudouOUC, +894/-54 9 files; 3 batched: ① ACP `writeTextFile/readTextFile` 走 PR 18 `WorkspaceFileSystem` 真关 PR 18 `ws.ts:613` TOCTOU 线 ② `closeSession/killSession` 用 `channelInfoForEntry(entry)` closes #4325 ③ 新 PR 18 primitive `WorkspaceFileSystem.writeTextOverwrite` 让 ACP writes 默认 0o600 + mode preservation + atomic temp+rename; **user-visible**: agent writes through symlinks 现 reject `symlink_escape`)
  - 🚧 **F2** [PR#4336](https://github.com/QwenLM/qwen-code/pull/4336) Wave 5 PR 23 shared MCP transport pool **WIP DRAFT 3/6 commits** 2026-05-19 23:52 (doudouOUC, +11848/-802 91 files, target `daemon_mode_b_main`; **解 #4306 揭示 "double MCP discovery in ACP child" 架构发现**; foundation only 无 daemon wiring; 161/161 tests; 10 关键不变式 (fingerprint key SHA-256, HTTP/SSE opt-in, spawnInFlight dedupe, drain grace + idle cap, snapshot replay on attach, generation guard, per-session trust copy, cross-platform pid sweep); 新工作流 "review at implement" — commit 2 后 background `code-reviewer` agent 抓 2 P1 bugs fold to commit 3; **existing standalone qwen 路径 untouched 71/71 tests pass unchanged**)
  - 🔧 **F3** [PR#4335](https://github.com/QwenLM/qwen-code/pull/4335) Wave 5 PR 24 PermissionMediator 4 strategies **OPEN REVIEW_REQUIRED** 2026-05-19 19:34 (doudouOUC, +9748/-517 62 files, target `daemon_mode_b_main`; 4 strategies (first-responder pre-F3 default 字节保 / designated / consensus N-of-M + partial vote SSE / local-only kernel-stamped); MultiClientPermissionMediator owns all state; 512-entry audit ring NOT on SSE; 5 hardness invariants 含 N3 deliberately preserved 旧 originatorClientId 不一致 保 wire 字节; pair-token + revocation API 推迟 follow-up)
