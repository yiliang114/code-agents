...
- 🎉 **Wave 1+2+2.5+3 完整 + Wave 4 过半 + 3 bonus spike 全 ship**（2026-05-16~18 共 **23 MERGED + 2 OPEN**：16 Wave PRs + 4 follow-up + 3 bonus adapter spike + Wave 4 PR 16/21 OPEN；Wave plan 进度 16/31 ≈ 52%；**无 block 点**）：
  ...
  - 🔧 Bonus Stage 1 wire-up 3 PR draft: [PR#4260](https://github.com/QwenLM/qwen-code/pull/4260) tui harness / [PR#4261](https://github.com/QwenLM/qwen-code/pull/4261) channel `--daemon-url` / [PR#4263](https://github.com/QwenLM/qwen-code/pull/4263) IDE smoke (chiga0, 2026-05-18 03:00 起)
...
- 🔧 [PR#4132](https://github.com/QwenLM/qwen-code/pull/4132) `/demo` debug page (jifeng) **仍 OPEN 4 天 24 reviews** — daemon 项目 OPEN 最长 PR；最新 CONFLICTING (与 #4247/4250/4251 冲突) + [Critical] XSS in `logEvent` tag (daemon-controlled `update.sessionUpdate.kind` 从 SSE 上行) —— 威胁模型扩到 daemon-emitted poisoned events
