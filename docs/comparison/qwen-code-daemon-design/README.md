...
- 🎉 **Wave 1+2+2.5+3+4 + W5 PR 22-series + F1/F2/F3 + multi follow-up**（2026-05-16~21 daemon main 侧 **38 MERGED + OPEN/draft/CLOSED 若干**；F-series 4 PR 全 MERGED 到 `daemon_mode_b_main`：F1 #4319 / F1 follow-up #4334 / F2 #4336 / F3 #4335；F4 prereq #4360 OPEN；Wave plan 进度 **22.75/31 ≈ 73%**，剩余按 F1-F5 重组）：
  ...
  - 🔧 **chiga0 第 3 PR — ACP HTTP transport 接入 ACP 生态** [PR#4472](https://github.com/QwenLM/qwen-code/pull/4472) `feat(daemon): ACP Streamable HTTP transport at /acp [RFD #721] (draft)` **OPEN REVIEW_REQUIRED DRAFT** 2026-05-24 01:14 (chiga0, +1967/-0 10 files, target `daemon_mode_b_main`); **在 `qwen serve` 加 official ACP Streamable HTTP transport ([RFD #721](https://github.com/agentclientprotocol/agent-client-protocol/pull/721)) 作第二 northbound transport** —— 挂载 `/acp` 单端点，与现有 REST+SSE API **共存**共享同一 `HttpAcpBridge`+`EventBus`；**让 Zed / Goose / future ACP-native SDK 可直接驱动 daemon**，不需要懂 qwen-specific REST 方言；3 关键决策：① dual-transport additive 非替换 (RFD #721 spec 自身仍 Draft 所有 SDK 没实现 + webui+3 SDK+VSCode 依赖 REST surface)，env `QWEN_SERVE_ACP_HTTP=0` opt-out ② extension namespace `_qwen/...` 用 ACP spec 保留的 `_` 前缀 (`_qwen/session/set_model` / `_qwen/session/heartbeat`)，`initialize` 中 advertise ③ deferred follow-up: WebSocket upgrade / HTTP/2 multiplexing / 完整 `fs/*`+`terminal/*` forwarding / SSE resumability parity; 新 `packages/cli/src/serve/acpHttp/` 含 jsonRpc/sseStream/connectionRegistry/dispatch; 15 vitest + live smoke 全绿)
