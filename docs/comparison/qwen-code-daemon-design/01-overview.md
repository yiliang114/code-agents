...
> - 🔧 **Bonus** Stage 1 developer harness / wire-up 3 PR：[PR#4260](https://github.com/QwenLM/qwen-code/pull/4260) `qwen daemon-tui` harness (+248) / [PR#4261](https://github.com/QwenLM/qwen-code/pull/4261) channel `--daemon-url` flag (+159/-43) / [PR#4263](https://github.com/QwenLM/qwen-code/pull/4263) IDE `Daemon Smoke Test` 命令 (+165) —— OPEN draft + CHANGES_REQUESTED (chiga0, 2026-05-18 03:00 起)
...
> - 🔧 **外部** `/demo` debug page [PR#4132](https://github.com/QwenLM/qwen-code/pull/4132) — **远古 OPEN 4 天 24 reviews** (jifeng, 2026-05-14 02:20, +810/-3)；单 HTML browser-based debug UI，**Mode B POST+SSE 最薄 client 验证面**；当前 CONFLICTING with main (PR#4247/4250/4251 改 `server.ts`) + [Critical] **XSS** in `logEvent` `tag` 是 daemon-controlled (`update.sessionUpdate.kind` / `event.type` 从 SSE 上行)；**威胁模型扩到 daemon-emitted poisoned events**
