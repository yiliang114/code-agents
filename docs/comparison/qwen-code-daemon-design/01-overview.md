...
> - 🔧 Wave 2 **PR 7** [PR#4231](https://github.com/QwenLM/qwen-code/pull/4231) daemon-stamped client identity — **OPEN** (chiga0, 2026-05-17 06:56, +672/-63 10 文件, 1.34x 测试比；header-based echo `X-Qwen-Client-Id` 防自报 + state-changing route gate + `originatorClientId` stamp + 新 capability tag `client_identity`；live-session scoped 无 revocation；⚠️ 未改 `integration-tests/cli/qwen-serve-routes.test.ts` 三套来源 lockstep 待补)
> - 🔧 PR 3 follow-up [PR#4225](https://github.com/QwenLM/qwen-code/pull/4225) DaemonSessionClient hardening — **OPEN** (chiga0, 2026-05-17 03:54, +254/-15, **测试 / 生产 = 2.06x**) eager subscription guard 改 call-time + `Last-Event-ID` 拒绝 invalid cursor
