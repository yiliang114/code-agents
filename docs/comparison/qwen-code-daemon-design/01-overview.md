...
> - 🔧 Wave 3 **PR 12** [PR#4241](https://github.com/QwenLM/qwen-code/pull/4241) read-only status routes — **OPEN** (doudouOUC, 2026-05-17 10:35, +2112/-61 19 文件 423 tests；5 routes：`/workspace/mcp` + `/skills` + `/providers` + `/session/:id/context` + `/session/:id/supported-commands`；**关键不变式**：workspace routes **不 spawn ACP** 即使 polled（idle workspace 返 `initialized: false`），避免 monitoring 保活 daemon；session routes 要求 live session；新 `status.ts` +173 LOC + 5 个新 capability tags + SDK 5 个 typed payload types；⚠️ 未含 integration-test 改动，三套来源 lockstep 风险（同 chiga0 PR#4231 同款，预测 wenshao 首轮抓）)
