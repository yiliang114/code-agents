...
>
> 🎉 **Wave 1+2+2.5 完整 + Wave 3/4 五 PR 同飞**（2026-05-16~17 共 **16 MERGED + 5 OPEN**：13 Wave PRs + 3 follow-up MERGED + Wave 3 PR 13/14 + Wave 4 PR 16/18/21 在飞累计 **+12832 LOC**；doudouOUC 单日 12 PR / 12.5h ≈ 1h/PR；Wave plan 进度 13/31 ≈ 42%；**无 block 点**）：
...
> - 🔧 Wave 3 **PR 13** [PR#4251](https://github.com/QwenLM/qwen-code/pull/4251) preflight + env diagnostics routes — **OPEN** (doudouOUC, 2026-05-17 15:49 回头补 PR 14 之前跳过的 PR 13；+1893/-29 19 文件 **720/720 tests passing**；2 route: `GET /workspace/env`（永远从 `process.*`，从不 consult ACP）+ `GET /workspace/preflight`（daemon-level cells 总渲染 / ACP-level cells idle 时 `not_started` placeholder）；**第一个 land closed `errorKind` 7-value taxonomy**：`missing_binary` / `blocked_egress` / `auth_env_error` / `init_timeout` / `protocol_error` / `missing_file` / `parse_error`（PR 14 #4247 将复用）；**5 strict invariants**：`env_var` cell 永不带 value（hard-assert）/ proxy URL 走 `redactProxyCredentials` + `URL.host` 只露 host:port / whitelist + presence-only enumeration 非白名单 secret 不可见 / `errorKind` closed enum 未识别 → undefined 不 fabricate / 两 route 永不 spawn ACP child（bridge test `handles.length === 0`）；6 commit 拆分；新 `BridgeTimeoutError` typed class + `mapDomainErrorToErrorKind` helper；out of scope: egress probe deferred to PR 14；⚠️ 未含 integration-tests，**第六次 drift 风险**)
> - 🔧 Wave 3 **PR 14** [PR#4247](https://github.com/QwenLM/qwen-code/pull/4247) MCP client guardrails — **OPEN** (doudouOUC, 2026-05-17 14:58 跳过 PR 13 直接挑 PR 14；+1188/-13 12 文件 **637/637 tests pass across 21 files**；**counter 在 core 不在 daemon 防 race**；3-mode 矩阵 `enforce`/`warn`/`enforce`：`warn` over-reserve 让 operator 看 `liveCount > budget`；**slot reservation by name** 在 `Set<string>` 抗 reconnect / health-monitor restart；atomic check + reserve before `await client.connect()` 防 `Promise.all` microtask 边界 interleave；新 capability tag `mcp_guardrails` modes `['warn', 'enforce']`；status `/workspace/mcp` 加 4 字段 + per-server `disabledReason: 'budget'`；`subprocessCount = stdio + websocket` 与 PR#4205 baseline `pgrep -P` 交叉验证；引 claude-code `filterMcpServersByPolicy` + opencode `heap.ts` hysteresis 等 4 prior art；v2 PR 14b deferred typed SSE push events；⚠️ 未含 `integration-tests/cli/qwen-serve-routes.test.ts`，**第四次 drift 风险**（PR#4245 刚立 invariant 又被）)
> - 🔧 Wave 4 **PR 16** [PR#4249](https://github.com/QwenLM/qwen-code/pull/4249) workspace memory + agents CRUD — **OPEN** (doudouOUC, 2026-05-17 15:06，**+2954/-3 17 文件 daemon 项目第二大 PR**；7 routes: `GET/POST /workspace/memory` + `GET/POST /workspace/agents` + `GET/POST/DELETE /workspace/agents/:agentType`；2 capability tag `workspace_memory` + `workspace_agents` + 2 typed event `memory_changed`/`agent_changed` workspace-level fan-out；3 scope decision: 静态 `QWEN.md` only（auto-memory deferred PR 16.5）/ agents POST split create+update（409 `agent_already_exists`）/ audit stamp `originatorClientId` only（persistent log deferred PR 24）；**工程亮点**：① CRUD-scoped `Config` Proxy stub fail-loud 防 future refactor silently incorrect ② whitespace-only-append suppression 防 no-op POST 污染 + 误导 fan-out（`writeContextFile` 返 `{ changed: boolean }`）③ workspace-level `bridge.publishWorkspaceEvent` + `knownClientIds()` snapshot；**PR open 前自跑 2 specialist agents (code-review + silent-failure-hunter) 5 项 high-impact 已修**——review pipeline 推前到 PR open 前；693 tests green；⚠️ **第五次 drift 风险**：未含 integration-tests，doudouOUC 故意 batch fix)
> - 🔧 Wave 4 **PR 18** [PR#4250](https://github.com/QwenLM/qwen-code/pull/4250) FileSystemService boundary — **OPEN** (doudouOUC, 2026-05-17 15:19，**+3270/-68 16 文件 daemon 项目曾最大 PR**（除 Stage 1 PR#3889）；**pure refactor** 无 new route 无 capability tag；新模块 `packages/cli/src/serve/fs/`：`paths.ts`/`errors.ts`/`policy.ts`/`audit.ts`/`workspaceFileSystem.ts`+10-case contract test；**8 个 security/correctness 防线**：suspicious path pattern（NTFS ADS / UNC / DOS device names / 三连续 dots）+ ENOTDIR explicit reject + chain-aware realpath + errno categorize（EACCES → `permission_denied` / ELOOP → `symlink_escape`）+ `recordAndWrap` always audit + `readText` MAX_READ_BYTES check BEFORE slurping core 防 OOM + `glob` symlink chain check aggregated `fs.denied` + `Intent` exhaustive `never` guard；**audit 隐私**：SHA-256-hashed paths + `QWEN_AUDIT_RAW_PATHS=1` opt-in passthrough + discriminator `kind`；**`trusted: false` 默认**防 forget-inject silently allow writes；**3 specialist agent pre-run 5 P0 + 1 P1 pre-fix**——review pipeline 升到 3 agent；引 **claude-code `permissions/filesystem.ts`** + **opencode `File.Service`** 2 prior art；测试 / 生产 ≈ **98%** (1613/1652 LOC, 101 unit + 10 contract tests, 411/411 serve passing)；**不触发 mirror drift**（pure refactor 无新 cap）)
