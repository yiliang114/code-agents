...
| 持久化关键状态 | SQLite + drizzle-orm（`session.sql.ts:SessionTable`）| 主线沿用 JSONL（PR#3739）；SQLite 用于外部 orchestrator 聚合 audit / permission decisions（详见 [§15 持久化栈](./15-orchestrator-multi-tenancy.md#四持久化栈大致方向)）|
...
- **多 session 由 External orchestrator spawn 多 daemon 实例**（`qwen-coordinator` 角色）—— **不在 qwen-code 主线路线图**，由商业平台 / k8s operator / 云厂商基于 daemon building block 实现，参考 [§14 设计对比](./14-single-vs-multi-session-design.md) / [§15 多租户配额](./15-orchestrator-multi-tenancy.md) / [§03 §8.2 orchestrator API](./03-http-api.md#82-orchestrator-层-apiexternal-reference-architecture)
